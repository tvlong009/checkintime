var app=angular.module("app",["ui.router","ngMaterial","ngMessages","ngResource","bw.paging","ngMdIcons","pascalprecht.translate","ngSanitize","ui.bootstrap","ui.bootstrap.datetimepicker","ngCsv"]).run(["$rootScope","$window",function(t,e){t.$on("$stateChangeSuccess",function(a,o,n,r,i){t.currentState=o.name,t.title=o.data&&o.data.pageTitle?o.data.pageTitle:"admin",document.title=t.title,t.goBack=function(){e.history.back()}})}]).directive("compareTo",function(){return{require:"ngModel",scope:{otherModelValue:"=compareTo"},link:function(t,e,a,o){o.$validators.compareTo=function(e){return e==t.otherModelValue},t.$watch("otherModelValue",function(){o.$validate()})}}}).factory("UtilService",function(){var t={format:"YYYY-MM-DD"};return t.parseDate=function(e){return is.not.empty(e)?moment(e).format(t.format):void 0},t.today=moment().format(t.format),t.lastWeek=moment().subtract(1,"week").format(t.format),t});
...
app.constant("CONSTANT",{apiUrl:"http://localhost:8080"});
app.config(["$stateProvider","$urlRouterProvider","$locationProvider",function(t,a,e){t.state("login",{url:"/login",templateUrl:"views/login.html",controller:"LoginCtrl",onEnter:function(){},data:{pageTitle:"Login"}}).state("change-password",{url:"/change-password",templateUrl:"views/change-password.html",controller:"ChangePasswordCtrl",onEnter:function(){},data:{pageTitle:"Change password"}}).state("admin",{"abstract":!0,url:"/admin",templateUrl:"views/root.html",controller:"RootCtrl",onEnter:function(){},data:{pageTitle:"home"}}).state("admin.tracking-staff",{url:"/tracking-staff",templateUrl:"views/dashboard.html",controller:"TrackingStaffCtrl",onEnter:function(){},data:{pageTitle:"staffs"}}).state("admin.tracking-staff-detail",{url:"/tracking-staff-detail/:action/:id",templateUrl:"views/detail.html",controller:"TrackingStaffDetailCtrl",onEnter:function(){},data:{pageTitle:"staff check detail"}}),a.otherwise("/login"),e.html5Mode(!0)}]);
app.config(["$provide","$httpProvider",function(t,e){t.factory("HttpInterceptor",["$q","$injector",function(t,e){return{request:function(e){var r=localStorage.getItem("token");return null!=r&&(e.headers["x-access-token"]=r),e||t.when(e)},requestError:function(e){return t.reject(e)},response:function(e){return e||t.when(e)},responseError:function(r){return 403===r.status?e.get("$state").transitionTo("login"):t.reject(r)}}}]),e.interceptors.push("HttpInterceptor")}]);
app.controller("RootCtrl",["$scope",function(o){}]);
app.controller("ChangePasswordCtrl",["$scope","$state","$timeout","RestApi","dialogServices",function(t,o,e,a,n){t.user={password:"",confirmPassword:"",email:""},t.auth=function(){a.auth().update(t.user).$promise.then(function(t){localStorage.setItem("token",t.token),o.go("admin.tracking-staff")})["catch"](function(t){e(function(){n.showAlertDialog("Error",t.message,"ok")},0)})}}]);
app.controller("LoginCtrl",["$scope","$state","$timeout","RestApi","dialogServices",function(e,t,o,a,n){e.user={username:"",password:""},localStorage.removeItem("token"),n.hide(),e.auth=function(){a.auth().save(e.user).$promise.then(function(e){localStorage.setItem("token",e.token),t.go("admin.tracking-staff")})["catch"](function(e){n.showAlertDialog("Error",e.message,"ok")})}}]);
app.controller("TrackingStaffCtrl",["$scope","$filter","RestApi","dialogServices","$timeout","socket","UtilService",function(t,a,e,n,c,i,s){function r(e){if(e)if(t.getArray=[],is.object(t.critical.dateStart)&&is.object(t.critical.dateEnd)){t.csvHeader=["No.","Name"];for(var n=0,c="YYYY-MM-DD",i=_.groupBy(e,function(t){return t.staffId.name}),r=[],o=moment(t.critical.dateStart),f=moment(t.critical.dateEnd),d=f.diff(o,"days")+1,u=1;u<=d;u++){var h=o.format(c);r.push(h),t.csvHeader.push("In - "+h),t.csvHeader.push("Out - "+h),o.add(1,"day")}_.each(i,function(e,c){var i={No:++n,name:c};_.each(r,function(t,n){var c=_.filter(e,function(a){return t===s.parseDate(a.dateIn)});c.length>0?(i["checkIn"+n]=a("date")(c[0].dateIn,"hh:mm:ss a"),i["checkOut"+n]=a("date")(c[0].dateOut,"hh:mm:ss a"),_.each(c,function(t,e){e>0&&(i["checkIn"+n]+="/"+a("date")(t.dateIn,"hh:mm:ss a"),i["checkOut"+n]+="/"+a("date")(t.dateOut,"hh:mm:ss a"))})):(i["checkIn"+n]="",i["checkOut"+n]="")}),t.getArray.push(angular.copy(i))})}else{t.csvHeader=["No.","Name","Email","Check In","Check out","Working time"];for(var m=0,l=e.length;m<l;m++){var u=e[m];t.getArray.push({no:m+1,name:u.staffId.name,email:u.staffId.email,checkIn:a("date")(u.dateIn,"yyyy/MM/dd hh:mm:ss a"),checkOut:a("date")(u.dateOut,"yyyy/MM/dd hh:mm:ss a"),workingTime:t.totalHours(u)})}}}t.size=[],t.size.push("all");for(var o=20;o<400;o+=20)t.size.push(o);t.timeTrackings=[],t.staffs=[],t.getArray=[];var f=function(){n.showLoadingDialog(),is.not.empty(t.critical.dateEnd)&&(t.critical.dateEnd=new Date(moment(t.critical.dateEnd).endOf("day").valueOf())),e.timeTracking().get(t.critical).$promise.then(function(a){t.timeTrackings=[],c(function(){t.timeTrackings=a.data,t.critical.total=a.total,n.hide(),r(t.timeTrackings)},0)})["catch"](function(t){c(function(){n.showAlertDialog("warning",t.data.message,"ok")},0)})};t.clear=function(){t.critical={staffId:"",dateStart:"",dateEnd:"",type:"",limit:20,total:0,pageNumber:1},f()},t.search=function(){f()},t.totalHours=function(t){return t.dateOut?moment(t.dateIn).preciseDiff(t.dateOut):"N/A"},t.csvHeader=[],t.getByPage=function(a){t.critical.pageNumber=a,f()},i.on("response-check-in",function(a){for(var e=a.staffId,c=0,i=t.staffs.length;c<i;c++)t.staffs[c]._id==e&&n.showNotificationToast(t.staffs[c].name+" check in")}),i.on("response-check-out",function(a){for(var e=a.staffId,c=0,i=t.staffs.length;c<i;c++)t.staffs[c]._id==e&&n.showNotificationToast(t.staffs[c].name+" check out")}),e.staff().get().$promise.then(function(a){t.staffs=a.data,t.clear()})["catch"](function(t){c(function(){n.showAlertDialog("warning",t.data.message,"ok")},0)})}]);
app.controller("TrackingStaffDetailCtrl",["$scope","$state","$timeout","RestApi","dialogServices","socket",function(t,e,a,n,i,o){t.action=e.params.action,t.timeTracking={dateIn:"",dateOut:"",staffId:""};var c=function(){i.showLoadingDialog(),n.timeTracking().get({id:e.params.id}).$promise.then(function(e){a(function(){var a=e.data;a.dateIn&&(a.dateIn=new Date(a.dateIn)),a.dateOut&&(a.dateOut=new Date(a.dateOut)),a.staff=a.staffId,a.staffId="",t.timeTracking=a,i.hide()},0)})["catch"](function(t){i.showAlertDialog("warning",t.data.message,"ok")})},g=function(){i.showLoadingDialog(),n.staff().get().$promise.then(function(e){t.staffs=e.data,a(function(){t.staffs=e.data,i.hide()},0)})["catch"](function(t){i.showAlertDialog("warning",t.data.message,"ok")})};"add"==t.action?g():c(),t.update=function(){return""!=t.timeTracking.dateOut&&new Date(t.timeTracking.dateOut).getTime()<new Date(t.timeTracking.dateIn).getTime()?i.showAlertDialog("warning","You must select date check out bigger than date check in","ok"):void n.timeTracking().updateTracking({id:t.timeTracking._id,dateIn:t.timeTracking.dateIn,dateOut:t.timeTracking.dateOut}).$promise.then(function(t){a(function(){i.showAlertDialog("Congratulation!!!",t.message,"ok").then(function(){o.emit("refresh-data"),e.go("admin.tracking-staff")})},0)})["catch"](function(t){i.showAlertDialog("warning",t.data.message,"ok")})},t.add=function(){if(t.timeTracking.staffId.length>0)if(""!=t.timeTracking.dateIn){if(""!=t.timeTracking.dateOut&&new Date(t.timeTracking.dateOut).getTime()<new Date(t.timeTracking.dateIn).getTime())return i.showAlertDialog("warning","You must select date check out bigger than date check in","ok");n.timeTracking().addTracking({staffId:t.timeTracking.staffId,dateIn:t.timeTracking.dateIn,dateOut:t.timeTracking.dateOut}).$promise.then(function(t){a(function(){i.showAlertDialog("Congratulation!!!",t.message,"ok").then(function(){o.emit("refresh-data"),e.go("admin.tracking-staff")})},0)})["catch"](function(t){i.showAlertDialog("warning",t.data.message,"ok")})}else i.showAlertDialog("warning","Please select date check in","ok");else i.showAlertDialog("warning","Please select staff","ok")},t.deleteTimeWorking=function(){n.timeTracking()["delete"]({id:t.timeTracking._id}).$promise.then(function(t){a(function(){i.showAlertDialog("Congratulation!!!",t.message,"ok").then(function(){o.emit("refresh-data"),e.go("admin.tracking-staff")})},0)})["catch"](function(t){i.showAlertDialog("warning",t.data.message,"ok")})},t.isOpen1=!1,t.isOpen2=!1,t.openCalendar=function(e,a){t["isOpen"+a]=!0}}]);
app.factory("dialogServices",["$mdDialog","$mdToast","$q","$translate",function(e,t,o,n){function i(t){return s=!0,e.show(t)}var l={},s=!1;return l.hide=function(){return s=!1,e.hide()},l.hasShowingDialog=function(){return s},l.showWaitingDialog=function(e){var t=o.defer(),l=t.promise;return l.onComplete=function(e){l.then(function(){e()})},n([e.title,e.message]).then(function(o){i({templateUrl:"views/dialog/waiting-dialog.html",targetEvent:e.targetEvent,controller:function(e,t){e.message=t.message,e.title=t.title},locals:{title:o[e.title],message:o[e.message]},bindToController:!0,clickOutsideToClose:!angular.isUndefined(e.clickOutsideToClose)&&e.clickOutsideToClose,escapeToClose:!angular.isUndefined(e.escapeToClose)&&e.escapeToClose,onComplete:function(){t.resolve()}})}),l},l.showAlertDialog=function(t,l,s,r){var a=o.defer();return n([t,l,s]).then(function(o){i(e.alert().title(o[t]).content(o[l]).ok(o[s]).targetEvent(r)).then(function(e){a.resolve(e)})}),a.promise},l.showNotificationToast=function(e){var i=o.defer(),l=i.promise;return l.onComplete=function(e){l.then(function(){e()})},n([e]).then(function(o){t.show({templateUrl:"views/dialog/simple-toast.html",controller:["$scope",function(t){t.message=o[e]}],hideDelay:1e3,onComplete:function(){i.resolve()}})}),l},l.showLoadingDialog=function(){return e.show({templateUrl:"views/dialog/loading.html",clickOutsideToClose:!0,escapeToClose:!0})},l.showConfirmDialog=function(e,t){var l=o.defer();return n([t.title,t.message]).then(function(o){i({templateUrl:"views/dialog/confirm-dialog.html",clickOutsideToClose:!1,targetEvent:e,escapeToClose:!0,locals:{title:o[t.title],message:o[t.message]},bindToController:!0,controller:function(e,t,o,n){e.title=o,e.message=n,e.yes=function(){t.hide(!0)},e.no=function(){t.hide(!1)}}}).then(function(e){l.resolve(e)})}),l.promise},l.checkTimeDialog=function(e){var t=o.defer();return i({templateUrl:"views/dialog/check-dialog.html",clickOutsideToClose:!0,targetEvent:e,escapeToClose:!0,bindToController:!0,controller:function(e,t){e.checkIn=function(){t.hide(!0)},e.checkOut=function(){t.hide(!1)}}}).then(function(e){t.resolve(e)}),t.promise},l}]);
app.factory("RestApi",["$resource",function(t){return{auth:function(){return t("/auth",{},{update:{method:"PUT"}})},staff:function(){return t("/api/staff/:staffId",{staffId:"@staffId"},{update:{method:"POST",url:"/api/staff/update-with-photo"}})},timeTracking:function(){return t("/api/time-tracking/:id",{id:"@id"},{getTime:{method:"GET",url:"/api/time-tracking/getTime",isArray:!1},update:{method:"PUT"},addTracking:{method:"POST",url:"/api/time-tracking/add-tracking"},updateTracking:{method:"POST",url:"/api/time-tracking/update-tracking"}})}}}]);
app.factory("socket",["$location",function(o){var t=o.protocol()+"://"+o.host()+":"+o.port();return io.connect(t)}]);